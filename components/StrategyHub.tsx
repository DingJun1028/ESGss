...
import { AlertTriangle, Users, TrendingUp, Globe, ShieldAlert, Target, Zap, ArrowRight, Layers, BrainCircuit } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          <h2 className="text-3xl font-bold text-white mb-2 tracking-tight">{t.modules.strategy.title}</h2>
        ...
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900/80 border border-white/10 backdrop-blur-md">
            ...
            <span className="text-xs text-emerald-400 font-medium font-mono">{isZh ? 'Omni-Component v2: ACTIVE' : 'Omni-Component v2: ACTIVE'}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.map((m, idx) => (
          <OmniEsgCell
            ...
            verified={m.verified}
            traits={m.traits as any} // Cast for demo
            tags={m.tags}
            dataLink={m.dataLink}
          />
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass-panel p-6 rounded-2xl relative overflow-hidden flex flex-col">
          ...
          <div className="relative flex-1 min-h-[350px] w-full bg-slate-900/30 rounded-xl border border-white/5 p-8 flex items-center justify-center">
             {/* Grid Lines */}
             <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 pointer-events-none p-8">
                <div className="border-r border-b border-white/5 border-dashed"></div>
                <div className="border-r border-b border-white/5 border-dashed"></div>
                <div className="border-b border-white/5 border-dashed"></div>
                <div className="border-r border-b border-white/5 border-dashed"></div>
                <div className="border-r border-b border-white/5 border-dashed"></div>
                <div className="border-b border-white/5 border-dashed"></div>
                <div className="border-r border-white/5 border-dashed"></div>
                <div className="border-r border-white/5 border-dashed"></div>
             </div>
            ...
            <div className="relative w-full h-full grid grid-cols-3 grid-rows-3 gap-4">
                {/* Axis Labels */}
                <div className="absolute -left-6 top-1/2 -rotate-90 text-[10px] font-bold text-gray-500 tracking-[0.2em] flex items-center gap-2">
                   <ArrowRight className="w-3 h-3" /> IMPACT
                </div>
                <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-gray-500 tracking-[0.2em] flex items-center gap-2">
                   PROBABILITY <ArrowRight className="w-3 h-3" />
                </div>
                
                {risks.map((risk, i) => (
                    <div key={i} className={`
                        relative group flex items-center justify-center rounded-xl border backdrop-blur-md cursor-pointer hover:scale-105 transition-all duration-300 z-10 shadow-lg
                    ...
                    `}>
                        ...
                        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 bg-slate-900/95 border border-white/20 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 p-3 text-center pointer-events-none shadow-2xl z-20 backdrop-blur-xl translate-y-2 group-hover:translate-y-0">
                            <div className="text-xs font-bold text-white mb-1 border-b border-white/10 pb-1">{risk.name}</div>
                            <div className="text-[10px] text-gray-400 mb-1">Scenario: RCP 8.5 (Business as Usual)</div>
                            <div className="flex items-center justify-center gap-1 text-[10px] text-emerald-400 bg-emerald-500/10 py-1 px-2 rounded-full">
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col h-full">
          ...
          <div className="mt-6 pt-6 border-t border-white/5">
             <button className="w-full py-3 rounded-xl bg-gradient-to-r from-celestial-gold/10 to-transparent border border-celestial-gold/30 text-sm text-celestial-gold hover:bg-celestial-gold/20 transition-all flex items-center justify-center gap-2 group relative overflow-hidden">
